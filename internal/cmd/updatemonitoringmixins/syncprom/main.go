...
import (
	...
	"github.com/rancher/ob-charts-tool/internal/cmd/updatemonitoringmixins/pythonish/re"
...
func PrometheusRulesSync(tempDir string) error {
	logrus.Info("Syncing prometheus rules")
	chartPath := config.GetContext().ChartRootDir
	chartsSources := constants.RulesSourceCharts()
	for _, chart := range chartsSources {
		currentState := chartState{}
		err := prepareGitRules(&currentState, tempDir, chart, chartPath)
		if err != nil {
			return err
		}
		common.SetDefaultMaxK8s(&chart)
		writeErr := writeOutput(currentState, chart)
		if writeErr != nil {
			return writeErr
		}
	}
...
func AddCustomLabels(rules string, group AlertGroup) string {
	condition := constants.RulesConditionMap[group.Name]
	ruleGroupLabels := getRuleGroupCondition(condition, "additionalRuleGroupLabels")

	baseLabelIndent := strings.Repeat(" ", indent+labelIndent)
	...
	labelSeperator := "\n" + strings.Repeat(" ", indent) + "  labels:"
	sectionSeperator := "\n" + strings.Repeat(" ", indent) + "  \\S"
	sectionSeperatorLen := len(sectionSeperator) - 1
	rulesPositions := re.FindIter(ruleSeperator, rules)
...
{{- end }}`
	...
	return textwrap.Indent(
		...
		strings.Repeat(" ", indent+labelIndent*2),
	)
}

func AddCustomAnnotations(rules string, group AlertGroup) string {
	ruleCondition := "{{- if .Values.defaultRules.additionalRuleAnnotations }}\n{{ toYaml .Values.defaultRules.additionalRuleAnnotations | indent 8 }}\n{{- end }}"
	groupCondition := constants.RulesConditionMap[group.Name]
	...
	separator := strings.Repeat(" ", indent) + "- alert:.*"
	alertsPositions := re.FindIter(separator, rules)
	alert := 0
...
func AddCustomKeepFiringFor(rules string) string {
	...
	separator := strings.Repeat(" ", indent) + "  for:.*"
	alertsPositions := re.FindIter(separator, rules)
	alert := 0
...
func addCustomAlertRules(rules, keyToReplace string) string {
	indentedKey := strings.Repeat(" ", indent) + keyToReplace
	alertPrefix := "- alert:"
	var (
...
func AddRulesConditionsFromConditionMap(rules string) string {
	return addRulesConditions(rules, constants.AlertConditionMap)
}

func addRulesConditions(rules string, conditionMap map[string]string) string {
	ruleCondition := "{{- if %s }}\n"
	lineStart := strings.Repeat(" ", indent) + "- alert: "
...
func AddRulesPerRuleConditions(rules string, group AlertGroup) string {
