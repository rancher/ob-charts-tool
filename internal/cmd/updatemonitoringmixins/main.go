...
func PrepareTempDir() (string, error) {
	...
	if err != nil {
		logrus.Error("Error creating temporary directory:", err)
		return "", err
	}
...
func UpdateMonitoringMixins(useCache bool) error {
	...
	// Clone git repos
	for _, repoConfig := range constants.Repos {
		var cloneErr error
		...
		if cloneErr != nil {
			logrus.Error("Error shallow cloning repository:", cloneErr)
			os.Exit(1)
		}
	}

	dashboardsErr := syncgrafana.DashboardsSync(tempDir)
	if dashboardsErr != nil {
		return dashboardsErr
	}

	rulesErr := syncprom.PrometheusRulesSync(tempDir)
	if rulesErr != nil {
		return rulesErr
	}
